...
begin
  ...
  {vDivisao := SimpleRoundTo((Soma / 11), -2);   //aredonda pra cima 0,54545 = 0,55

  vFrac :=  FormatFloat('#,##0.00', Frac(vDivisao));

  if vfrac[4] = '5' then
  begin
    vfrac[4] := '6';
    vResto := Round(strtofloat(vfrac )*10);
  end
  else
    vResto := Round((vDivisao - trunc(vDivisao))*10);   }
...
//        toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
    ...
    //remessa.Delete(remessa.Count-1);

    //remessa.Strings[remessa.Count -1] := StringReplace(remessa.Strings[remessa.Count-1] , #13#10, '', [rfReplaceAll]);
    //remessa.Text := StringReplace(remessa.Text , #13#10, '', [rfReplaceAll]);

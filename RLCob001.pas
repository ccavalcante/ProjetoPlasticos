...
begin
  ...
  begin
    if Trim(Cedente.ContaBancaria.DigitoAgencia) = '' then
      AAgenciaCodigoCedente := Formatar(Cedente.ContaBancaria.CodigoAgencia,4,false,'0') + '.' + Formatar(Cedente.CodigoCedente,8,false,'0') + '.' + Cedente.DigitoCodigoCedente
    else
      AAgenciaCodigoCedente := Formatar(Cedente.ContaBancaria.CodigoAgencia,4,false,'0') + '-' + Cedente.ContaBancaria.DigitoAgencia + '/' + Formatar(Cedente.CodigoCedente,8,false,'0') + '-' + Cedente.DigitoCodigoCedente;
...
//      lnC7_N10: ANossoNumero := Formatar(Cedente.ContaBancaria.Convenio,7,false,'0') + Formatar(NossoNumero,10,false,'0') + '-' + DigitoNossoNumero; {Removido conforme instruções de cso.curitiba.convenio@bb.com.br suporte técnico do banco do brasil
    ...
    ACarteira := Formatar(Carteira,2);
    Atitulo.Cedente.Nome := Atitulo.Cedente.Nome; // + ' - ' + '07.572.627/0001-66';
...
begin
   ...
   begin
      ...
      Registro := Registro + Formatar( Titulos[NumeroRegistro].Cedente.ContaBancaria.Convenio + '001417019  ',18,false,'0') + '  '; //33 a 52 - Código do convênio no banco [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
      ...
      Registro := Registro + '000';  // Formatar('0',3); // 226 a 228 - Uso exclusivo de Vans
      ...
      Registro := Registro + '00000000' ; // Formatar('0',8); //200 a 207 - Data do crédito - Só para arquivo retorno
      ...
      begin
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.Convenio,7, True,'0');
         Registro := Registro + Formatar(inttostr(strtoint(Titulos[NumeroRegistro].NossoNumero)),10,false,'0');                   // Formatar(Titulos[NumeroRegistro].NossoNumero,20,True,'0'); //38 a 57 - Nosso número - identificação do título no banco
         Registro := Registro + '   '; //55 - 3 espaços branco
         ...
         Registro := Registro + '2'; // '1'; //60 - Tipo de documento: Tradicional
         //*********** Emissão Boleto *****************************************
...
//         Registro := Registro + Formatar(Titulos[NumeroRegistro].SeuNumero,15,false,'0'); //63 a 77 - Número que identifica o título na empresa
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NumeroDocumento,12,false,'0') + '   '; //63 a 77 - Número que identifica o título na empresa [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
         ...
         begin
           if Titulos[NumeroRegistro].DescricaoOcorrenciaOriginal = 'U' then
           begin
             Registro := Registro + '2';//221 - Código de protesto: protesto em dias uteis
             Registro := Registro + Formatar( Titulos[NumeroRegistro].MotivoRejeicaoComando,2,false,'0');
           end
           else
           begin
         ...
         begin
            Registro := Registro + '3'; //221 - Código de protesto: nao protestar
            Registro := Registro + Formatar('',2,false,'0'); //222 a 223 - Prazo para protesto (em dias corridos)
         end;
         //**********************  Codigo p/ Baixa/Devolucao  ******************

         Registro := Registro + '0'; //224 - Campo não tratado pelo BB [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
         Registro := Registro + '000'; //225 a 227 - Campo não tratado pelo BB [ Alterado conforme instruções da CSO Brasília ] {27-07-09}
          //********************************************************************
         Registro := Registro + '09'; //228 a 229 - Código da moeda: Real
         Registro := Registro + '0000000000'; // Formatar('',10); //230 a 239 - Uso exclusivo FEBRABAN/CNAB
         ...
         Registro := Registro + '000'; //Formatar('',3); {Uso exclusivo FEBRABAN/CNAB}
        ...
        begin
           Registro := Formatar(CodigoBanco,3,false,'0'); {Código do banco}
           Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {Lote de Serviço}
           Registro := Registro + '3'; {Tipo do registro: Registro detalhe}
           if (Trim(Titulos[NumeroRegistro].Instrucoes.Text) = '') then
             Registro := Registro + Formatar(IntToStr(3 * NumeroRegistro +3), 5, false, '0') {Número seqüencial do registro no lote - Cada título tem 2 registros (P, Q ou R ou S)}
           else
             Registro := Registro + Formatar(IntToStr(4 * NumeroRegistro +3), 5, false, '0');
           Registro := Registro + 'R'; {Código do segmento do registro detalhe}
           Registro := Registro + ' '; {Uso exclusivo FEBRABAN/CNAB: Branco}
           case Titulos[NumeroRegistro].TipoOcorrencia of {Código de movimento Remessa}
            toRemessaRegistrar                 : Registro := Registro + '01';
            toRemessaBaixar                    : Registro := Registro + '02';
            toRemessaConcederAbatimento        : Registro := Registro + '04';
            toRemessaCancelarAbatimento        : Registro := Registro + '05';
           ...
           Registro := Registro + Formatar('',10);//Informação ao Sacado
           Registro := Registro + Formatar('',40);//Mensagem 3
           Registro := Registro + Formatar('',40);//Mensagem 4
           Registro := Registro + Formatar('',50);//E-mail sacado p/ envio de informações
      ...
      Registro := Registro + '5'; // 8 {Tipo do registro: Registro trailer do lote}
      Registro := Registro + Formatar('',9); // 09 a 17 {Uso exclusivo FEBRABAN/CNAB}
      ...
      Registro := Registro + Formatar(IntToStr(Remessa.Count),6,false,'0'); // 18 a 23
      {Totalização da cobrança simples - Só é usado no arquivo retorno}
      Registro := Registro + '000000' ; // Formatar('',6,false,'0');           // 24      {Quantidade títulos em cobrança}
      Registro := Registro + '00000000000000000'; //Formatar('',17,false,'0'); // 30 a 46 {Valor dos títulos em carteiras}
      {Uso exclusivo FEBRABAN/CNAB}
      Registro := Registro + '00000000000000000000000';  // Formatar('',23); // 47 a 69  {Uso exclusivo FEBRABAN/CNAB}
      {Totalização da cobrança caucionada - Só é usado no arquivo retorno}
      Registro := Registro + '000000'; // Formatar('',6,false,'0'); // 70 a 75 {Quantidade títulos em cobrança}
      Registro := Registro + '00000000000000000'; // Formatar('',17,false,'0'); // 76 a 93 {Valor dos títulos em carteiras}
      {Uso exclusivo FEBRABAN/CNAB}
      Registro := Registro + '00000000000000000000000'; // Formatar('',31); // 94 a 115 {Uso exclusivo FEBRABAN/CNAB}
      Registro := Registro + Formatar('',125); {Uso exclusivo FEBRABAN/CNAB}
...
//          toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
      ...
      if (Titulos[NumeroRegistro].DataProtesto <> 0) and (Titulos[NumeroRegistro].DataProtesto > Titulos[NumeroRegistro].DataVencimento) then  // Protesto
      begin

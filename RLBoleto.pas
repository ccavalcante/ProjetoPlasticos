...
uses
   ...
   RLBLayout,Contnrs, MaskUtils, DateUtils
  {$IFDEF VER150}
...
{ @abstract(Classe para gerar código de barras para boletos) }
   ...
   public
      ...
      procedure SavetoPDF(cod, pLayout :string);
...
uses RLCob000, RLCob001, RLCob033, RLCob104, RLCob237, RLCob275, RLCob341, RLCob399,
  RLCob409, RLCob356, RLCob748, RLCob756, uDM;
...
begin
  ...
  begin
    ...
    if Cedente.ContaBancaria.Banco.Codigo = '033' then //Santander Banespa
      txtNumeroBanco1.Caption := Cedente.ContaBancaria.Banco.Codigo+ '-7'
    else
    ...
    txtNomeCedente2.Caption := AnsiUpperCase(Cedente.Nome) + AnsiUpperCase(Cedente.NumeroCPFCGC);
...
begin
  ...
  begin
    ...
    try
      ...
      ABoleto.LayoutBoleto.savetofile('1.pdf');
      ABoleto.Free;
     except
      ABoleto.Free;
      raise;
  ...
  begin
    ...
    Agrupamento[0].LayoutBoleto.savetofile('1.pdf');
...
procedure TRLBTitulo.SavetoPDF( cod, pLayout :string );
var
  ABoleto : TLayoutBoleto;
  ABoleto2 : TFrmRLBLayout;
  DataSource: TDataSource;
  nI: Integer;
begin
  if nQtdeBoletos = -1 then
  begin
    if pLayout = 'S' then // 25/04/2017 salvar pdf em formato carne
    begin
      ABoleto2 := TFrmRLBLayout.Create(nil);
      try
        DataSource := TDataSource.Create(nil);
        DataSource.DataSet := FCDS;
        if BoletoLayout = blCarne then
        begin
          ABoleto2.BoletoCarne.DataSource := DataSource;
          ABoleto2.BoletoCarne.Prepare;
          ABoleto2.BoletoCarne.SaveToFile(cod);
        end
        else
        begin
          ABoleto2.BoletoPadrao.DataSource := DataSource;
          ABoleto2.BoletoPadrao.Prepare;
          ABoleto2.BoletoPadrao.SaveToFile(cod);
        end;
        ABoleto2.Free;
      except
        ABoleto2.Free;
    ...
    begin
      ...
      try
        PrepararBoleto(ABoleto);
        ABoleto.RLBand1.Visible := Tag = 0;
        ABoleto.LayoutBoleto.savetofile( cod);
        ABoleto.Free;
      except
        ABoleto.Free;
       raise;
  ...
  begin
    Agrupamento[0].RLBand1.Visible := Tag = 0; //quebra galho, impressao de boleto PRO 05/05/2005
    Agrupamento[0].LayoutBoleto.savetofile(cod);
    for nI := 0 to nQtdeBoletos do
      Agrupamento[nI].Free;
    nQtdeBoletos := -1;
    SetLength(Agrupamento, 0);
...
begin
  ...
  begin
    ...
    FCDS.FieldDefs.Add('CpfCnpjCedente'   ,ftString,30);
    FCDS.CreateDataSet;
  end;

  FCDS.Append;
  FCDS.FieldByName('NumeroBanco').AsString       := Cedente.ContaBancaria.Banco.Codigo + '-' + Cedente.ContaBancaria.Banco.Digito;
  ...
  if BoletoLayout = blCarne then
    begin
     case cedente.TipoInscricao of
      tiPessoaFisica  : FCDS.FieldByName('CpfCnpjCedente').AsString := FormatarComMascara('!000\.000\.000\-00;0; ',cedente.fNumeroCPFCGC);
      tiPessoaJuridica: FCDS.FieldByName('CpfCnpjCedente').AsString := FormatarComMascara('!00\.000\.000\/0000\-00;0; ',cedente.fNumeroCPFCGC);
      tiOutro         : FCDS.FieldByName('CpfCnpjCedente').AsString := cedente.fNumeroCPFCGC;
     end;
    end
  else
    begin
     case cedente.TipoInscricao of
      tiPessoaFisica  : FCDS.FieldByName('CpfCnpjCedente').AsString := 'CPF: ' + FormatarComMascara('!000\.000\.000\-00;0; ',cedente.fNumeroCPFCGC);
      tiPessoaJuridica: FCDS.FieldByName('CpfCnpjCedente').AsString := 'CNPJ: '+ FormatarComMascara('!00\.000\.000\/0000\-00;0; ',cedente.fNumeroCPFCGC);
      tiOutro         : FCDS.FieldByName('CpfCnpjCedente').AsString := cedente.fNumeroCPFCGC;
     end;
    end;

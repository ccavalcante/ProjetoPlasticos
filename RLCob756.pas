...
uses
  classes, SysUtils, math, RLBoleto
  {$IFDEF VER140}
...
begin
  ...
  begin
    AAgenciaCodigoCedente :=
       ...
       Formatar(Cedente.CodigoCedente + Cedente.DigitoCodigoCedente ,7,false,'0');
...
begin
  ...
  begin
    ...
    Registro := Registro + Formatar('',20); // Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,20,false,'0'); //33 a 52 - Código do convênio no banco
    ...
    Registro := Registro + '0';//72 - Dígito verificador da agência / conta
    Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.Nome,30,true,' '); //73 a 102 - Nome do cedente
    Registro := Registro + Formatar('SICOOB',30,true,' '); //103 a 132 - Nome do banco
    ...
    Registro := Registro + '081'; //164 a 166 - Número da versão do layout do arquivo
    ...
    Registro := Registro + '   '; // 223 a 225 - 'CSP'
    Registro := Registro + Formatar('',3); // 226 a 228 - Uso exclusivo de Vans
    ...
    Registro := Registro + '  '; //12 a 13 - Forma de lançamento: preencher com ZEROS no caso de cobrança
    ...
    Registro := Registro + Formatar('',20); // Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,20,false,'0'); //34 a 53 - Código do convênio no banco
    ...
    begin
      ...
      Registro := Registro + Formatar(IntToStr({2}3*NumeroRegistro+1),5,false,'0'); //9 a 13 - Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)
      ...
      //nosso numero
      Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,9,True,'0') + Formatar(Titulos[NumeroRegistro].DigitoNossoNumero,1,True,'0') ; //38 a 57 - Nosso número - identificação do título no banco
       Registro := Registro + '01'; //38 a 57 (11-12) parcela unica
       Registro := Registro + '01'; //38 a 57 (13-14) modalidade
       Registro := Registro + '1';  //38 a 57 (15-15) tipo 1,3,4,6
       Registro := Registro + Formatar('',5); //38 a 57 (16-20) tipo

      Registro := Registro + '1'; //58 - Cobrança Simples
      Registro := Registro + '0'; //59 - Forma de cadastramento do título no banco: com cadastramento
      Registro := Registro + ' '; //60- Tipo de documento: Tradicional
      //*********** Emissão Boleto *****************************************
      ...
      //*************************  Mora Juros  ******************************
      if (Titulos[NumeroRegistro].ValorMoraJuros > 0) and (Titulos[NumeroRegistro].ValorDocumento > Titulos[NumeroRegistro].ValorMoraJuros) then
      begin
      ...
      Registro := Registro + '0'; //224 - Código para baixa/devolução: Não baixar/não devolver
      Registro := Registro + '   ' ; //225 a 227 - Prazo para baixa/devolução (em dias corridos)
      ...
      Registro := Registro + Formatar('0000000000',10); //230 a 239 - Uso exclusivo FEBRABAN/CNAB
      ...
      Registro := Registro + Formatar(IntToStr({2}3*NumeroRegistro+2),5,false,'0'); {Número seqüencial do registro no lote - Cada título tem 2 registros (P e Q)}
      ...
      //begin
        ...
        Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); //4 a 7 {Número do lote}
        Registro := Registro + '3'; // 8 {Tipo do registro: Registro detalhe}
        Registro := Registro + Formatar(IntToStr(3*NumeroRegistro+3),5,false,'0'); //9-13 {Número seqüencial do registro no lote - Cada título tem 3 registros (P e Q)}
        Registro := Registro + 'R'; {14 - Código do segmento do registro detalhe}
        Registro := Registro + ' '; {15 Uso exclusivo FEBRABAN/CNAB: Branco}
        Registro := Registro + '01'; {16-17 codigo remessa}
        Registro := Registro + '0'; // 18 - nao conceder desconto 2
        Registro := Registro + '00000000'; //19-26 // data desconto 2
        Registro := Registro + '000000000000000'; // 27-41 vlr desc 2
        Registro := Registro + '0'; // 42 nao conceder desconto 3
        Registro := Registro + '00000000'; // 43-50 data desconto 3
        Registro := Registro + '000000000000000'; // 51-65 vlr desc 3
        Registro := Registro + '1'; { 66 - codigo remessa}
        Registro := Registro + FormatDateTime('ddmmyyyy',Titulos[NumeroRegistro].DataVencimento); // 67-74
        Registro := Registro + FormatCurr('000000000000000',Titulos[NumeroRegistro].ValorRecebido * 100); // 75-89
        Registro := Registro + Formatar('',10); {90-99 Nome do sacador/avalista}
        Registro := Registro + Formatar('',40); {100-139 Nome do sacador/avalista}
        Registro := Registro + Formatar('',40); {140-179 Nome do sacador/avalista}
        Registro := Registro + Formatar('',20); {180-199 Nome do sacador/avalista}
        Registro := Registro + '00000000';  //200-207 data limite
        Registro := Registro + '000';;   // 208-210 banco debito
        Registro := Registro + '00000';   //211-215 agencia debito
        Registro := Registro + Formatar(' ',1);   //216 dig debito
        Registro := Registro + '000000000000';   //217-228 conta debito
        Registro := Registro + Formatar(' ',1);   // 229 dig debito
        Registro := Registro + Formatar(' ',1);   // 230 dig debito
        Registro := Registro + '0'; // 231 - aviso debito automatico
        Registro := Registro + Formatar(' ',9);   //232-240 dig debito
        Remessa.Add(Registro);
        Registro := '';
      //end;

      //Fim Nilton 31/01/2023
    ...
    Registro := Registro + Formatar('',6,true,'0'); {Quantidade títulos em cobrança}
    Registro := Registro + Formatar('',17,true,'0'); {Valor dos títulos em carteiras}
    ...
    Registro := Registro + Formatar('',8, true, ' '); {Uso exclusivo FEBRABAN/CNAB}
    Registro := Registro + Formatar('',117, true, ' '); {Uso exclusivo FEBRABAN/CNAB}
    ...
    Registro := Registro + '000000'; {Uso exclusivo FEBRABAN/CNAB}
...
begin
  ...
  begin
    ...
    Remessa.Add('0'+ // IDENT.DO REGISTRO
                ...
                'COBRANÇA' + // IDENT.POR EXTENSO TIPO SERVIÇO
                ...
                Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,8,false,' ')+ // IDENT.DO CEDENTE NO BANCO
                Formatar(Titulos[NumeroRegistro].Cedente.DigitoCodigoCedente,1,false,' ')+ // IDENT.DO CEDENTE NO BANCO digito
                Formatar('',6)+ // USO DO BANCO branco
...
//        toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.NumeroConta,8,false,' ');  //Conta Corrente:
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoConta,1,false,' '); //Dígito Verificador da Conta:
      Registro := Registro + '000000'; //Número do Convênio de Cobrança do Beneficiário: "000000"
      Registro := Registro + '                         '; // 25 BRANCOS

      Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero ,11,false,'0'); // 063 - IDENT. DO TITULO NO BANCO COM DIG VERIFICADOR
      Registro := Registro + Formatar(Titulos[NumeroRegistro].DigitoNossoNumero ,1,true,'0'); //74 -DIG VERIFICADOR

      Registro := Registro + '0100'; // parcela 01 +// grupo valor '00'
      Registro := Registro + '    '; // BRANCOS

      Registro := Registro + '   '; // PREFIXO DO TITULO
      Registro := Registro + '000'; // VARIAÇÃO DA CARTEIRA
      ...
      Registro := Registro + '    '; // BRANCOS
      Registro := Registro + '2'; // tipo de emissão 1 cooperativa 2 cliente
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,4,false,'0'); //// PREFIXO DA AGENCIA COBRADORA
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia,1,false,'0'); // DIGITO DA AGENCIA COBRADORA

      Registro := Registro + Formatar(aTitulosEspecieDocumento,2,false,'0'); // ESPÉCIE DO TITULO
      Registro := Registro + '1'; // IDENT. DE ACEITE DO TITULO
      ...
      Registro := Registro + '00';
      Registro := Registro + FormatCurr('000000',Titulos[NumeroRegistro].ValorMoraJuros * 10000); // VALOR DE MORA A SER COBRADO POR DIA DE ATRASO  ex: 022000 = 0,20%
      Registro := Registro + FormatCurr('000000',Titulos[NumeroRegistro].ValorRecebido * 10000); // taxa de multa
      Registro := Registro + '2'; //tipo distribuição 1- cooperativa 2- cliente;
      ...
      Registro := Registro + '9' + FormatCurr('000000000000',Titulos[NumeroRegistro].ValorIOF * 100); // VALOR DE IOF    193 cod moeda 9 = real
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Sacado.Endereco.Bairro,15); // Bairro do sacado
      ...
      Registro := Registro + FormatCurr('00', (Titulos[NumeroRegistro].DataProtesto - Titulos[NumeroRegistro].DataVencimento)); // DIAS PARA PROTESTO
      Registro := Registro + ' '; // BRANCOS

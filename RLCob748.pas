...
begin
   ...
   ANumeroConta   := Formatar(ATitulo.Cedente.CodigoCedente,5,false,'0');  // tava ContaBancaria.numeroConta
   ANossoNumero   := ATitulo.NossoNumero; // Formatar(ATitulo.NossoNumero,5,false,'0');

   ADigitoNossoNumero := Modulo11(ACodigoAgencia + APostoCedente + ANumeroConta +
                         ANossoNumero) ;  // FormatDateTime('yy',Date) + '2' + ANossoNumero);
...
begin
   ...
   With ATitulo do begin
      ...
      ANossoNumero := NossoNumero + CalcularDigitoNossoNumero(ATitulo);
...
begin
   With ATitulo do begin
      ...
      ANossoNumero:= copy(NossoNumero, 1, 2 )+ '/' + copy(NossoNumero, 3, 6 ) + '-' + DigitoNossoNumero;
...
var
   ...
   vMultaBol : string;
begin
 // if DM.ThR000MULTA.Value > 0 then
 //   vMultaBol := StringReplace(StringReplace(formatfloat('00.00', DM.ThR000MULTA.Value),',', '', [rfReplaceAll]), '.', '', [rfReplaceAll] )
 // else
    vMultaBol := '0000';
   ...
   with ACobranca do  begin
      ...
      Remessa.Add('0'+ // IDENT.DO REGISTRO
                  ...
                  Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,5,true,'0')+ // IDENT.DO CEDENTE NO BANCO
                  Formatar(Titulos[NumeroRegistro].Cedente.NumeroCPFCGC,14,true,'0')+ // IDENT.DO CNPJ
                  Formatar('', 31)+ // VAZIO
...
//          toRemessaAlterarSeuNumero          : ATitulosTipoOcorrencia := '08';
        ...
        case Titulos[NumeroRegistro].AceiteDocumento of
         ...
         Registro := Registro + 'AA'; // “A” - Sicredi Com Registro e “A” – Simples
         if Titulos[NumeroRegistro].BoletoLayout = blPadrao then // “A” – Normal “B” – Carnê
            Registro := Registro + 'A'
         else
            Registro := Registro + 'B';
         Registro := Registro + Formatar('',12);
         Registro := Registro + 'AAA'; // “A” – Real /“A” – Valor “B” – Percentual / “A” – Valor “B” – Percentual
         Registro := Registro + Formatar('',28);
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,8,true,'0') + Formatar(Titulos[NumeroRegistro].DigitoNossoNumero,1,true,'0') ; // IDENT. DO TITULO NO BANCO COM DIG VERIFICADOR
         Registro := Registro + Formatar('',06);
         Registro := Registro + FormatDateTime('yyyymmdd',Titulos[NumeroRegistro].DataProcessamento); // DATA DE VENCIMENTO DO TITULO
         Registro := Registro + Formatar('',01);
         Registro := Registro + Formatar('N',01);
         Registro := Registro + Formatar('',01);
         Registro := Registro + Formatar('B',01); //“A”–Imp. é feita Sicredi, “B”–Imp. é feita Beneficiário
         Registro := Registro + Formatar('',04);
         Registro := Registro + Formatar('',04);
         Registro := Registro + FormatCurr('0000000000',Titulos[NumeroRegistro].ValorDesconto); // VALOR DESCONTO A SER CONCEDIDO
         Registro := Registro + FormatCurr('0000',Titulos[NumeroRegistro].ValorRecebido);//vMultaBol;  // multa
         Registro := Registro + Formatar('',12);
         Registro := Registro + Formatar(aTitulosEspecieDocumento,02) ; // cada. titulo
         ...
         Registro := Registro + Formatar('',09);
         Registro := Registro + 'A'; // dupl. mercantil
         Registro := Registro + Formatar(aTitulosAceite,1); // IDENT. DE ACEITE DO TITULO
         Registro := Registro + FormatDateTime('ddmmyy',Titulos[NumeroRegistro].DataDocumento); // DATA DE EMISSAO DO TITULO //DataProcessamento
         if Titulos[NumeroRegistro].DataProtesto=0 then begin
            Registro := Registro + '0000';
         end
         else begin
            Registro := Registro + '0603'; // 3 dias uteis, 4 dias corridos
         end;
         ...
         Registro := Registro + inttostr(strtoint(ASacadoTipoInscricao)); //1 PF, 2 PJ
         ...
         Registro := Registro + '00000'; // CODIGO do cleinte
         Registro := Registro + Formatar('',14); // USO DO BANCO
         Registro := Registro + Formatar('',41); // USO DO BANCO 
         Registro := Registro + Formatar(IntToStr(NumeroRegistro+2),6,false,'0') ; // '000002' NUMERO DO BORDERO
      ...
      Remessa.Add('91748'+ // IDENT. DO REGISTRO
                  Formatar( Titulos[0].Cedente.CodigoCedente,5,true,'0') +

...
begin
   ...
   begin
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {Número da convênio}
      ...
      Registro := Registro + '050'; {Número da versão do layout do arquivo}
      ...
      Registro := Registro + '030'; {Número da versão do layout do lote}
      ...
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {convênio/cedente}
      Registro := Registro + Formatar('0',14,false,'0'); {Uso exclusivo CAIXA}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.CodigoAgencia,5,false,'0'); {Código da agência do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.ContaBancaria.DigitoAgencia,1,false,'0'); {Dígito da conta do cedente}
      Registro := Registro + Formatar(Titulos[NumeroRegistro].Cedente.CodigoCedente,6,false,'0'); {convênio/cedente}
      ...
      Registro := Registro + Formatar('0',8, false, '0'); {Data do crédito - Só para arquivo retorno}
      ...
      begin
         ...
         begin
           if (Trim(Titulos[NumeroRegistro].Instrucoes.Text) <> '') and (Titulos[NumeroRegistro].ValorRecebido > 0)  then // se não existe S ou R
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NossoNumero,15,false,'0'); {Nosso número - identificação do título no banco}
         Registro := Registro + '1'; { código carteira cobrança simples - segundo manual}
         if (Titulos[NumeroRegistro].Carteira = '14') or (Titulos[NumeroRegistro].Carteira = 'CR') or (Titulos[NumeroRegistro].Carteira = '90')then
           Registro := Registro + '1'  { forma de cadastramento do título pelo banco cobrança registrada - segundo manual}
         else
           Registro := Registro + '2'; { forma de cadastramento do título pelo banco cobrança sem registro - segundo manual}
         Registro := Registro + '2';{tipo título}
         ...
         Registro := Registro + Formatar(Titulos[NumeroRegistro].NumeroDocumento,11,false,'0'); {Número que identifica o título na empresa}
         ...
         Registro := Registro + '0'; {Dígito da agência cobradora}
         ...
         Registro := Registro + Formatar(Formatar(Titulos[NumeroRegistro].NumeroDocumento,11,false,'0'),25); {Identificação do título na empresa}
         ...
         Registro := Registro + Formatar(IntToStr(NumeroLote),4,false,'0'); {Número do lote}
         Registro := Registro + '3'; {Tipo do registro: Registro detalhe}
         if (Trim(Titulos[NumeroRegistro].Instrucoes.Text) = '') and (Titulos[NumeroRegistro].ValorRecebido = 0) then //verificando se não existe o Segmento S e R
           ...
           if (Trim(Titulos[NumeroRegistro].Instrucoes.Text) <> '') and (Titulos[NumeroRegistro].ValorRecebido > 0) then //se existe os S e R
             ...
             Registro := Registro + Formatar(IntToStr(3 * NumeroRegistro +2),5,false,'0'); //existe só um
         ...
         begin
           ...
           Registro := Registro + '000000000000000';//Valor/Percentual a Ser Concedido
           Registro := Registro + '2';//Código da Multa  1 = valor 2 = percental
         ...
         begin
           ...
           Registro := Registro + '3'; {Tipo do registro: Registro detalhe}
           if Titulos[NumeroRegistro].ValorRecebido > 0 then
             Registro := Registro + Formatar(IntToStr(4 * NumeroRegistro + 4), 5, false, '0') {Número seqüencial do registro no lote - Cada título tem 2 registros (P, Q  ou R ou S)}
           else
             Registro := Registro + Formatar(IntToStr(3 * NumeroRegistro + 3), 5, false, '0'); {Número seqüencial do registro no lote - Cada título tem 2 registros (P, Q ou R ou  S)}
      ...
      Registro := Registro + Formatar(IntToStr(vQtdTit),6,false,'0'); {Quantidade títulos em cobrança}
      Registro := Registro + Formatar(FloattoStr(vValTit*100) ,17,false,'0'); {Valor dos títulos em carteiras}
      {Totalização da cobrança caucionada }
      Registro := Registro + Formatar('0',6,false,'0'); {Quantidade títulos em cobrança}
      Registro := Registro + Formatar('0',17,false,'0'); {Valor dos títulos em carteiras}
